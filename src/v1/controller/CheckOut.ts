...
router.put("/updateAddress", addressValidations, userAuth, updateAddress);
...
async function updateAddress(req: any, res: any, next: any) {
  try {
    const checkoutObj = new Checkoutdb();
    const token = res.locals.user;
    const { sh_address1, sh_address2, sh_city, sh_state, sh_country, sh_pincode } = req.body;
    if (!token) {
      return res.status(401).send("Token not get..");
    }
    else {
      let result: any = await checkoutObj.updateAddress(token, sh_address1, sh_address2, sh_city, sh_state, sh_country, sh_pincode);
      if (result?.error) {
        return res.send(functionObj.output(0, result.message));
      }
      return res.send(functionObj.output(1, result.message, result.data));
