...
async function addUpdateCart(req: any, res: any, next: any) {
  try {
    ...
    let result: any = await Cartobj.addUpdateCart(product_id, cart_quantity, token, ip);
    if (result?.error) {
      return res.send(functionObj.output(0, result.message));
    }
    return res.send(functionObj.output(result.status_code, result.message, result.data));
  } catch (error) {
    next(error);
    return;
  }
