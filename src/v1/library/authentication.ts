...
export async function userAuth(req: any, res: any, next: any): Promise<any> {
  ...
  try {
    ...
    if (!token) {
      return res.status(401).json({ status_code: 0, status_message: "USER IS NOT AUTHENTICATE..." });
    }
    ...
    if (verifyToken && verifyToken.id) {
      ...
      return res.status(401).json({ status_code: 0, status_message: "No Authorized Request" });
    }
  } catch (err) {
    console.error("JWT Verification Error:", err);
    return functionObj.output(0, "INTERNAM ERROR IN AUTH MIDDLEWARE");
  }
}
...
export async function AdminAuth(req: any, res: any, next: any): Promise<any> {
  ...
  if (!token) {
    return res.status(300).json({ status_code: 0, status_message: "ADMIN NOT AUTHENTICATE..." });
  }
  const secret_key = process.env.JWT_SECRET || "asdfghjklzxcvbnm";
  const verifyToken = jwt.verify(token, secret_key) as JwtPayload;
