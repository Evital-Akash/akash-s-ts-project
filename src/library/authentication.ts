...
export async function userAuth(req: any, res: any, next: any): Promise<any> {
  ...
  try {
    ...
    if (!token) {
      return res.status(300).json({ status_code: 0, status_message: "Not Authenticate user.." });
    }
...
export async function AdminAuth(req: any, res: any, next: any): Promise<any> {
  ...
  if (!token) {
    return res.status(300).json({ status_code: 0, status_message: "Not Authenticate Admin" });
  }
